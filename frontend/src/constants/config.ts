import type { Live2DModelKey, Live2DModelSpec } from '@/types/avatar'
...
export const RUNTIME_RESOURCES: RuntimeResource[] = [
  ...
  ...Object.entries(LIVE2D_MODELS).map(([key, model]) => ({
    key: `live2d-${key}`,
    title: `Live2D ${model.label}`,
    description: `主界面可切换头像模型，已内置到 ${model.url}。`,
    actions: [{ label: '查看模型文件', path: model.url }],
  })),
  {
    key: 'opencv-assets',
    title: 'OpenCV 面部追踪',
