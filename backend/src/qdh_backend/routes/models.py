...
async def model_download_handler(payload: DownloadRequest):
    del payload
    return _unsupported("后端不再下载本地模型文件；请使用 Ollama 管理 LLM 模型，例如 ollama pull <model>。")
...
async def model_preload_asr_handler():
    return _unsupported("ASR 由浏览器 SpeechRecognition / webkitSpeechRecognition 提供，无需后端预热。")
